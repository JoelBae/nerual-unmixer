...
class OTTSTFTProxy(nn.Module):
    ...
    def __init__(self, sr=44100, duration=2.0, downsample_factor=64):
        ...
        # Phase 2: Frequency Domain Spectrogram Corrector
        # Input: (Batch, Real/Imag, Freq, Time) -> (B, 2, 513, T)
        self.residual_net = nn.Sequential(
            ...
            nn.GELU(),
            nn.Conv2d(32, 2, kernel_size=(3, 3), padding=(1, 1))
        )
        
        # Initialize final layer to pass-through (zero correction)
    ...
    def forward(self, audio, params):
        ...
        stft_correction = self.residual_net(stft_input)
